...
async def handle_notifications_menu(message: Message, app):
    ...
    await message.answer("<b>⚙️ Налаштування сповіщень:</b>\nОберіть потрібні типи сповіщень та пороги:", parse_mode=ParseMode.HTML, reply_markup=kb)
...
async def toggle_start_notify(message: Message, app):
    ...
    await message.answer(f"Початок друку: {'Увімкнено ✅' if user['notify']['start'] else 'Вимкнено ❌'}", reply_markup=kb)
...
async def toggle_finish_notify(message: Message, app):
    ...
    await app.storage.save_user(user)
    kb = get_notify_keyboard(user["notify"])
    await message.answer(f"Закінчення друку: {'Увімкнено ✅' if user['notify']['finish'] else 'Вимкнено ❌'}", reply_markup=kb)
...
async def toggle_clear_notify(message: Message, app):
    ...
    await app.storage.save_user(user)
    kb = get_notify_keyboard(user["notify"])
    await message.answer(f"Нагадування зняти деталь: {'Увімкнено ✅' if user['notify']['remind_clear'] else 'Вимкнено ❌'}", reply_markup=kb)
...
async def handle_time_notify_menu(message: Message, app):
    ...
    await message.answer("⏳ <b>Оберіть час для запевного сповіщення до кінця друку:</b>", parse_mode=ParseMode.HTML, reply_markup=time_kb)

@router.message(F.text.in_(["⏳ 5 хв до кінця", "⏳ 10 хв до кінця", "⏳ 15 хв до кінця", "❌ Вимкнути таймер сповіщень"]))
async def handle_set_time_notify(message: Message, app):
    ...
    await app.storage.save_user(user)
    kb = get_notify_keyboard(user["notify"])
    msg_str = f"Встановити сповіщення за {t_val} хв до кінця ✅" if t_val > 0 else "Попереднє сповіщення за часом вимкнено ❌"
...
async def handle_filament_notify_menu(message: Message, app):
    ...
    fil_kb = ReplyKeyboardMarkup(keyboard=[
    ...
    await message.answer("📦 <b>Оберіть поріг залишку нитки для попередження:</b>", parse_mode=ParseMode.HTML, reply_markup=fil_kb)
...
async def handle_set_filament_notify(message: Message, app):
    ...
    await app.storage.save_user(user)
    kb = get_notify_keyboard(user["notify"])
    msg_str = f"Попереджувати, коли нитки менше {f_val}g ✅" if f_val > 0 else "Попередження за лімітом нитки вимкнено ❌"
    await message.answer(msg_str, reply_markup=kb)
