...
async def handle_cancel_or_back(message: Message, app):
    ...
    if state in ["add_p_name", "add_p_ip", "add_p_code", "add_p_sn"]:
        ...
        await message.answer("Додавання принтера скасовано! Передумав вже?! 😤", reply_markup=get_printers_keyboard(app.printers))
        return
    ...
    if target_printer and (state in ["edit_filament_weight", "confirm_stop_print", "confirm_delete_printer"] or text in ["назад", "⬅️ назад"]):
