...
async def handle_commercial_menu(message: Message, app):
    ...
    txt = (
        f"<b>💰 Комерційний калькулятор ціни</b>\n"
        f"<i>Налаштування себевартості та маржі для комерційного друку</i> 💼✨\n\n"
        f"<b>📋 Наявні пресети:</b>\n"
    )
...
COMMERCIAL_STATES = {
    "add_preset_name", "add_preset_price", "add_preset_elec", "add_preset_depr",
    "add_preset_cons", "add_preset_profit", "select_preset_to_copy", "copy_preset_new_name",
    "select_preset_to_edit", "edit_preset_field", "select_preset_to_delete",
    "calc_enter_weight", "calc_enter_time", "calc_select_preset", "edit_preset_field_value"
}
...
async def handle_commercial_wizard(message: Message, app):
    ...
    if st == "add_preset_name":
        ...
        await app.storage.save_user(user)
        await message.answer("Step 3/6: Введіть ціну електроенергії грн/кВт·год (наприклад: <code>4.32</code>):", parse_mode=ParseMode.HTML)
        return True
        ...
        await message.answer(f"✅ <b>Пресет «{html.escape(np['name'])}» успішно збережено!</b>", parse_mode=ParseMode.HTML)
        await handle_commercial_menu(message, app)
        return True
    ...
    # 2. COPY PRESET WIZARD
        ...
        await app.storage.save_user(user)
        await message.answer(f"📋 Копіюємо <b>«{html.escape(target['name'])}»</b>.\nВведіть нову назву для скопійованого пресету:", parse_mode=ParseMode.HTML)
        return True
    ...
    # 3. EDIT PRESET WIZARD
        ...
        inline_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"🧵 Ціна пластику ({target['price_per_g']}грн/г)", callback_data=f"edit_p_field_price_{target['id']}")],
            [InlineKeyboardButton(text=f"⚡ Світло ({target['electricity_rate_uah']}грн)", callback_data=f"edit_p_field_elec_{target['id']}")],
            [InlineKeyboardButton(text=f"🔧 Амортизація ({target['depreciation_val']})", callback_data=f"edit_p_field_depr_{target['id']}")],
            [InlineKeyboardButton(text=f"🧼 Витратники ({target['consumables_val']})", callback_data=f"edit_p_field_cons_{target['id']}")],
            [InlineKeyboardButton(text=f"💼 Прибуток ({target['profit_val']})", callback_data=f"edit_p_field_profit_{target['id']}")]
        ])
        await message.answer(f"✏️ <b>Редагування пресету: {html.escape(target['name'])}</b>\nОберіть параметр для зміни:", parse_mode=ParseMode.HTML, reply_markup=inline_kb)
        return True
    ...
    # 4. DELETE PRESET
        ...
        user["state"] = "idle"
        await app.storage.save_user(user)
        await message.answer(f"🗑️ Пресет <b>«{html.escape(target['name'])}»</b> успішно видалено!", parse_mode=ParseMode.HTML)
        await handle_commercial_menu(message, app)
        return True
    ...
    # 5. QUICK CALC WIZARD
        ...
        await app.storage.save_user(user)
        await message.answer("⏱️ Введіть орієнтовний час друку у хвилинах (наприклад: <code>180</code> для 3 годин):", parse_mode=ParseMode.HTML)
        return True
        ...
        kb = [[KeyboardButton(text=p["name"])] for p in presets.values()]
        kb.append([KeyboardButton(text="⬅️ Назад")])
        await message.answer("📋 <b>Оберіть пресет для розрахунку:</b>", parse_mode=ParseMode.HTML, reply_markup=ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True))
        return True
    ...
    # 6. INLINE FIELD EDIT VALUE
        ...
        user["state"] = "idle"
        await app.storage.save_user(user)
        await message.answer(f"✅ <b>Параметр успішно оновлено для «{html.escape(p['name'])}»!</b>", parse_mode=ParseMode.HTML)
        await handle_commercial_menu(message, app)
        return True
